...
class IsOwnCompany(permissions.BasePermission):
    """Company can only access their own CompanyProfile."""
...
class IsParentCompany(permissions.BasePermission):
    """Company can only access institutions where parent_company == user.company_profile."""
...
class IsInstitutionAccess(permissions.BasePermission):
    """
    Company: can access institution if parent_company == user.company_profile.
    Institution: can access only their own institution.
    """
    ...
    def has_permission(self, request, view):
        if not request.user or not request.user.is_authenticated:
            return False
        return getattr(request.user, 'role', None) in ('company', 'institution')
...
class CanViewRequest(permissions.BasePermission):
    """
    Company: request.receiving_company == user.company_profile.
    ...
    def has_permission(self, request, view):
        ...
        return role in ('company', 'institution')
